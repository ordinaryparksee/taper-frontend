...
const searchOpenedKnowledge = ref<Knowledge>()
...
function getRowItems(row: Row<Knowledge>): DropdownMenuItem[] {
  return [
    ...
    },
    {
      ...
      async onSelect() {
        searchOpenedKnowledge.value = row.original
...
<template>
  ...
  <div v-else class="flex flex-col gap-3">
    ...
    <UModal
      v-if="searchOpenedKnowledge"
      v-model:open="isOpenSearch"
      fullscreen
      title="Try Search"
      :description="searchOpenedKnowledge.name"
    >
      <template #body>
        <KnowledgeSearch
          :knowledge="searchOpenedKnowledge"
          class="h-full"
        />
      </template>
    </UModal>
