...
const { data, status, refresh } = await useApi<PaginatedList<Conversation>>('/conversations', {
  query: {
    ...params.value,
  ...
  return [
    ...
    { type: 'separator' },
    {
      ...
      async onSelect() {
        ...
        await $api(`/conversations/${row.original.code}`, { method: 'DELETE' })
        await refresh()
        toast.add({
          title: 'Success',
...
<template>
  <UEmpty
    ...
    description="It looks like you haven't started any conversations."
    :icon="app.ui.icons.conversation"
    variant="naked"
    :actions="[
      { icon: app.ui.icons.plus, label: 'Create new', to: '/conversations/@new' },
